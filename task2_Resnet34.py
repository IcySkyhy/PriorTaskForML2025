...
import numpy as np
import math
from model import CUB200ResNet34  # 使用ResNet-34
...
def get_cosine_schedule_with_warmup(optimizer, num_warmup_epochs, num_training_epochs, eta_min=1e-6):
    """余弦退火学习率调度器，带warmup，确保学习率不会降到0"""
...
def main():
    # ==================== 配置参数 ====================
    BATCH_SIZE = 32          # 批次大小 (ResNet-34更大，可能需要减小batch)
    EPOCHS = 150             # 训练轮数 (更深的网络需要更多epoch)
    ...
    print("=" * 80)
    print("CUB-200 Classification with ResNet-34 (Training from Scratch)")
    print("=" * 80)
    ...
    # ==================== 初始化模型（从头训练） ====================
    print("\n" + "-" * 80)
    print("Initializing ResNet-34 model from SCRATCH (random weights)...")
    print("-" * 80)
    
    # 创建模型 - 使用自定义的ResNet-34，完全随机初始化
    model = CUB200ResNet34(num_classes=200, input_size=IMAGE_SIZE[0]).to(DEVICE)
    
    # 验证权重是随机初始化的
    print("\nVerifying random initialization:")
    first_conv_weight = model.conv1.weight.data
    print(f"  First conv layer weight mean: {first_conv_weight.mean().item():.6f}")
    print(f"  First conv layer weight std: {first_conv_weight.std().item():.6f}")
    print("  (Non-zero std confirms random initialization)")
    ...
    # 余弦退火学习率调度器
    scheduler = get_cosine_schedule_with_warmup(optimizer, WARMUP_EPOCHS, EPOCHS, eta_min=1e-6)
    ...
    for epoch in range(EPOCHS):
        ...
        if (epoch + 1) % 20 == 0:
            torch.save({
                ...
                'best_acc': best_acc,
                'history': history,
            }, f"cub200_resnet34_epoch{epoch+1}.pth")
    ...
    # 加载最佳模型进行最终评估
    checkpoint = torch.load("cub200_resnet34_best.pth")
    ...
    # 保存最终模型（仅权重）
    torch.save(model.state_dict(), "cub200_resnet34_final_weights.pth")
    print(f"\nFinal weights saved to cub200_resnet34_final_weights.pth")
